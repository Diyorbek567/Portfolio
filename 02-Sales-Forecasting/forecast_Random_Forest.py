...
# plt.show()
file.drop(columns=['Номенклатура','index'],inplace=True)
#prognozlash
# print(file['Размер'])
...
for current_size in razmer:
    ...
    for dizayn, guruh in size_group.groupby('Дизайн'):
        ...
        #print('yaxshi')
        #if len(filtered_data) <= 6:continue

        #print(filtered_data)
        # x=filtered_data.drop(columns=['Период','Размер','nomenklatura', 'Дизайн'])
        # y=filtered_data['nomenklatura']
        #filtered_data['Период'] = pd.to_datetime(filtered_data['Период'])
        train_set = filtered_data[filtered_data['year-month'] < '2024-12']
        test_set = filtered_data[filtered_data['year-month'] >= '2024-12']
        X_train = train_set.drop(columns=['year-month','Размер','nomenklatura', 'Дизайн','Коллекция'])
        X_test = test_set.drop(columns=['year-month','Размер','nomenklatura', 'Дизайн','Коллекция'])
        ...
        next_month_data = pd.DataFrame({
                    ...
                    #'margin_lag1': [filtered_data['margin_lag1'].iloc[-1]],
                })

        columns_order = ['Себестоимость', 'design_sales_mean',
                                 'collection_sales_mean', 'price', 'Year', 'Month', 
                                 'lag1', 'lag2','lag3', 
                                 'lag1_n', 'lag2_n','lag3_n','sezon_high','sezon_low','sezon_mid']
                
                # Moslashtirish
        next_month_data = next_month_data[columns_order]
        ...
        # 2024-12 uchun haqiqiy nomenklatura qiymatini olish
        real_nomenklatura = test_set[
            (test_set['Размер'] == current_size) & 
            (test_set['Дизайн'] == dizayn)
        ]['nomenklatura']

        # Agar mos keluvchi qiymat bo‘lsa, birinchisini olamiz, aks holda None
        nomenklatura_value = real_nomenklatura.iloc[0] if not real_nomenklatura.empty else None

        # Natijani forecasts ro‘yxatiga qo‘shish
        kolleksiya = filtered_data['Коллекция'].iloc[-1] if 'Коллекция' in filtered_data.columns else None
        forecasts.append((current_size, kolleksiya, dizayn, next_month_forecast, nomenklatura_value))


print("Yaxshi ketyapti")
# Natijalarni DataFramega aylantirish
forecast_df = pd.DataFrame(forecasts, columns=['Размер','Коллекция', 'Дизайн','Next_Month_Forecast','Real_Nomenklatura'])
...
forecast_df = forecast_df[['Размер','Дизайн','Letters','Numbers', 'rank_f', 'rank_real', 'Next_Month_Forecast', 'Real_Nomenklatura']]
...
# 1-sheet: Evaluation
evaluation_df = forecast_df[['Размер','Дизайн','Letters','Numbers', 'rank_f', 'rank_real', 'Next_Month_Forecast', 'Real_Nomenklatura', 'rank_difference']]

# 2-sheet: Collection-based comparison
collection_forecast = forecast_df[['Letters', 'Next_Month_Forecast', 'Real_Nomenklatura']].copy()
collection_sums = collection_forecast.groupby('Letters')[['Next_Month_Forecast', 'Real_Nomenklatura']].sum()
total_model = collection_sums['Next_Month_Forecast'].sum()
total_real = collection_sums['Real_Nomenklatura'].sum()
collection_sums['Model (%)'] = (collection_sums['Next_Month_Forecast'] / total_model * 100).round(2)
collection_sums['Real (%)'] = (collection_sums['Real_Nomenklatura'] / total_real * 100).round(2)
# RANKING
collection_sums['Model Rank'] = collection_sums['Model (%)'].rank(method='min', ascending=False).astype(int)
collection_sums['Real Rank'] = collection_sums['Real (%)'].rank(method='min', ascending=False).astype(int)

collection_percentage_df = collection_sums[['Model (%)', 'Real (%)', 'Model Rank', 'Real Rank']].sort_values(by='Model Rank').reset_index()

# 📁 Faylga yozish
output_file = "forecast_results.xlsx"
with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
    evaluation_df.to_excel(writer, sheet_name="Evaluation", index=False)
    collection_percentage_df.to_excel(writer, sheet_name="nomenklatura Percents", index=False)
